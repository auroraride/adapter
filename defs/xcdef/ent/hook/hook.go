...
import (
	...
	"github.com/auroraride/adapter/defs/xcdef/ent"
)
...
// The ReignFunc type is an adapter to allow the use of ordinary
// function as Reign mutator.
type ReignFunc func(context.Context, *ent.ReignMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ReignFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ReignMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ReignMutation", m)
}
